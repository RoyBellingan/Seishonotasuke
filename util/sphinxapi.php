...
class SphinxClient
{
	...
	var $res;			///< la risorsa, ultima cercata
	var $index;
	...
	/*
	 * Configuro per l'uso "normale" l'indice della bibbia
	 * 
	 */
	function config_1_bibbia($modo=SPH_MATCH_EXTENDED2)
	{
		...
		switch ($lang) {
			case "pусский":
				$this->index="index_russian_p";
				break;

			case "español":
				$this->index="index_espanol_p";
				break;
				
			case "日本語":
				$this->index="index_nihongo_p";
				break;

			default:
				$this->index="index_".$lang."_p";
		}
		if ($lang=="")
		{
	...
	function getquery($q,$o,$l)
	{
		...
		$max=$o+$l;
		$this->SetLimits ( $o, $l, $max, 0 );
		$res = $this->Query ( $q, $this->index );
		$this->res=$res;

		if ( $res===false)
		{
			print "Query failed: " . $this->GetLastError() . ".\n";
			return false;
