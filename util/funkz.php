...
function strippa2 ($selettati,$valore){
	if (!$selettati)
	{// primo ciclo, perchè è vuoto il campo
		$selettati="`" .$valore. "`";
	}
	else
	{// dal secondo in poi si mette la virgola
...
function cancella_conf($findme,$verbose=false){
	...
	$file='Util/config.php';         // Apro il file di configurazione
	$fr = fopen($file, 'r+');		 // In lettura + scrittura, con il puntatore all'inizio del file

	$my_file=file_get_contents($file);	// E lo leggo
	$lenght=strlen($findme);			// quanto è lunga
	$pos=strpos($my_file,$findme);		//cerco la cosa da cancellare
...
function checknote($nota)
{
	...
	end: //sintatti valida, da errore devi aggiornare l'editor a php 5.3 (goto)
