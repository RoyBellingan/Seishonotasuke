...
/*
 * 
 * Full list of options/settings can be found at the bottom of this file and at
...
(function($) {
	...
	$.fn.cluetip = function(js, options) {
		...
		return this.each(function(index) {
			...
			// support metadata plugin (v1.0 and 2.0)
				var opts = $.extend(true, {}, $.fn.cluetip.defaults, options
						|| {}, $.metadata ? $this.metadata() : $.meta ? $this
						.data() : {});
				...
				if (!$('#cluetip').length) {
					$( [
							...
							'</div>' ].join(''))[insertionType](
							insertionElement).hide();
				...
				var dropShadowSteps = (opts.dropShadow) ? +opts.dropShadowSteps
						: 0;
				if (!$dropShadow) {
					$dropShadow = $( []);
				...
				// content that will be displayed in the clueTip
				...
				var tOffset = parseInt(opts.topOffset, 10), lOffset = parseInt(
						opts.leftOffset, 10);
				// vertical measurement variables
				var tipHeight, wHeight, defHeight = isNaN(parseInt(opts.height,
						10)) ? 'auto' : (/\D/g).test(opts.height) ? opts.height
						: opts.height + 'px';
				var sTop, linkTop, posY, tipY, mouseY, baseline;
				// horizontal measurement variables
				var tipInnerWidth = parseInt(opts.width, 10) || 275, tipWidth = tipInnerWidth
						+ (parseInt($cluetip.css('paddingLeft'), 10) || 0)
						+ (parseInt($cluetip.css('paddingRight'), 10) || 0)
						+ dropShadowSteps, linkWidth = this.offsetWidth, linkLeft, posX, tipX, mouseX, winWidth;

				// parse the title
				var tipParts;
				var tipTitle = (opts.attribute != 'title') ? $this
						.attr(opts.titleAttribute) : '';
				if (opts.splitTitle) {
				...
				if (opts.escapeTitle) {
					tipTitle = tipTitle.replace(/&/g, '&amp;').replace(/>/g,
							'&gt;').replace(/</g, '&lt;');
				}
				...
				var activate = function(event) {
					...
					if (opts.positionBy == 'fixed') {
						...
						posX = (linkWidth > linkLeft && linkLeft > tipWidth)
								|| linkLeft + linkWidth + tipWidth + lOffset > winWidth ? linkLeft
								- tipWidth - lOffset
						...
						if (link.tagName.toLowerCase() == 'area'
							...
							if (mouseX + 20 + tipWidth > winWidth) {
								...
								posX = (mouseX - tipWidth - lOffset) >= 0 ? mouseX
										- tipWidth
										- lOffset
										- parseInt($cluetip.css('marginLeft'),
												10)
										+ parseInt($cluetipInner
												.css('marginRight'), 10)
										: mouseX - (tipWidth / 2);
							} else {
								posX = mouseX + lOffset;
						...
						$cluetip
								.css( {
									left : (posX > 0 && opts.positionBy != 'bottomTop') ? posX
											: (mouseX + (tipWidth / 2) > winWidth) ? winWidth
													/ 2 - tipWidth / 2
													: Math
															.max(
																	mouseX
					...
					/***********************************************************
					 * load the title attribute only (or user-selected
					 * attribute). clueTip title is the string before the first
					 * delimiter subsequent delimiters place clueTip body text
					 * on separate lines
					 **********************************************************/
						...
						if (tpl > 1) {
							for ( var i = 1; i < tpl; i++) {
								$cluetipInner
										.append('<div class="split-body">' + tipParts[i] + '</div>');
							}
						}
						cluetipShow(pY);
					}
					 ...
					 **********************************************************/
						...
						if (/\.(jpe?g|tiff?|gif|png)$/i.test(tipAttribute)) {
							$cluetipInner.html('<img src="' + tipAttribute
									+ '" alt="' + tipTitle + '" />');
							cluetipShow(pY);
							...
							var ajaxSettings = {
								...
								beforeSend : function(xhr) {
									if (optionBeforeSend) {
										optionBeforeSend.call(link, xhr,
								...
								error : function(xhr, textStatus) {
									if (isActive) {
										if (optionError) {
											optionError.call(link, xhr,
													textStatus, $cluetip,
													$cluetipInner);
										} else {
											$cluetipInner
													.html('<i>sorry, the contents could not be loaded</i>');
										}
									}
								},
								success : function(data, textStatus) {
									cluetipContents = opts.ajaxProcess.call(
											link, data);
									if (isActive) {
										if (optionSuccess) {
											optionSuccess.call(link, data,
													textStatus, $cluetip,
													$cluetipInner);
										}
										$cluetipInner.html(cluetipContents);
								...
								complete : function(xhr, textStatus) {
									if (optionComplete) {
										optionComplete.call(link, xhr,
												textStatus, $cluetip,
												$cluetipInner);
									}
									imgCount = $('#cluetip-inner img').length;
									if (imgCount && !$.browser.opera) {
										$('#cluetip-inner img').bind(
												'load error', function() {
													imgCount--;
													if (imgCount < 1) {
														$cluetipWait.hide();
							...
							var ajaxMergedSettings = $.extend(true, {},
									opts.ajaxSettings, ajaxSettings);
				...
				// get dimensions and options for cluetip and prepare it to be
				// shown
				var cluetipShow = function(bpY) {
					$cluetip.addClass('cluetip-' + ctClass);
					if (opts.truncate) {
						var $truncloaded = $cluetipInner.text().slice(0,
								opts.truncate) + '...';
					...
					; // empty function
					tipTitle ? $cluetipTitle.show().html(tipTitle)
							: (opts.showTitle) ? $cluetipTitle.show().html(
									'&nbsp;') : $cluetipTitle.hide();
					if (opts.sticky) {
						var $closeLink = $('<div id="cluetip-close"><a href="#">' + opts.closeText + '</a></div>');
						(opts.closePosition == 'bottom') ? $closeLink
								.appendTo($cluetipInner)
								: (opts.closePosition == 'title') ? $closeLink
										.prependTo($cluetipTitle) : $closeLink
										.prependTo($cluetipInner);
					...
					tipHeight = defHeight == 'auto' ? Math.max($cluetip
							.outerHeight(), $cluetip.height()) : parseInt(
							defHeight, 10);
					tipY = posY;
					...
					if (opts.positionBy == 'fixed') {
						...
						tipY = (tipHeight >= wHeight) ? sTop : baseline
								- tipHeight - tOffset;
					...
					if (opts.delayedClose > 0) {
						closeOnDelay = setTimeout(cluetipClose,
								opts.delayedClose);
					}
				...
				if ((/click|toggle/).test(opts.activation)) {
					$this.bind('click.cluetip', function(event) {
						if ($cluetip.is(':hidden')
								|| !$this.is('.cluetip-clicked')) {
							activate(event);
							$('.cluetip-clicked')
									.removeClass('cluetip-clicked');
					...
					// clicking is returned false if clickThrough option is set
					// to false
					...
					var mouseTracks = function(evt) {
						if (opts.tracking == true) {
							...
							var trackY = tipY ? tipY - evt.pageY : posY
									- evt.pageY;
	 ...
	 * options for clueTip
	 * 
	 * each one can be explicitly overridden by changing its value. for example:
	 * $.fn.cluetip.defaults.width = 200; would change the default width for all
	 * clueTips to 200.
	 * 
	 * each one can also be overridden by passing an options map to the cluetip
	 * method. for example: $('a.example').cluetip({width: 200}); would change
	 * the default width to 200 for clueTips invoked by a link with class of
	 * "example"
	 * 
	 */
	...
	$.fn.cluetip.defaults = { // set up default options
		...
		// whether to cache results of ajax request to avoid unnecessary hits to
		// server
		ajaxCache : true,
		...
		// can pass in standard $.ajax() parameters. Callback functions, such as
		// beforeSend,
		// will be queued first within the default callbacks.
		// The only exception is error, which overrides the default
		ajaxSettings : {
			// error: function(ct, ci) { /* override default error callback */ }
			// beforeSend: function(ct, ci) { /* called first within default
			// beforeSend callback }
			dataType : 'html'
		},
		debug : false
	 ...
	 * 
	 * @example $.cluetip.setup({ insertionType: 'prependTo', insertionElement:
	 * '#container' });
	 * 
	 * @property @name $.cluetip.setup @type Map @cat Plugins/tooltip @option
	 * String insertionType: Default is 'appendTo'. Determines the method to be
	 * used for inserting the clueTip into the DOM. Permitted values are
	 * 'appendTo', 'prependTo', 'insertBefore', and 'insertAfter' @option String
	 * insertionElement: Default is 'body'. Determines which element in the DOM
	 * the plugin will reference when inserting the clueTip.
	 * 
	 */
